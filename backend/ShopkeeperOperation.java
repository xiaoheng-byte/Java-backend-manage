...
public class ShopkeeperOperation {
    ...
    //订单管理
    //去发货
    public static void updateOrderInfo(String order_id,String shopkeeper_id){//这里的shopkeeperid来自登陆时的信息
    	    	conn = Connect.getConnection();
    	    	String sql = "update OrderInfo set state = '已发货'  where shopkeeper_id = '" + shopkeeper_id + "' and state = '已下单' and order_id ='" + order_id + "'";
    	    	try {
    	    		pstmt = conn.prepareStatement(sql);
    	        	pstmt.executeUpdate();
    ...
    public static JTable selectTotalNumAndTotalPriceOfOrderInfoByTime(String t_from, String t_to, String shopkeeper_id){
    	conn = Connect.getConnection();
    	String sql = "select count(distinct order_id) as '该时间段订单数', sum(total_price) as '该时间段订单总金额'  from OrderInfo where state != '已取消' and shopkeeper_id = '" + shopkeeper_id + "' and order_time > '" + t_from + "' and order_time < '" + t_to + "'" ;
    	try {
    		pstmt = conn.prepareStatement(sql);
        	ResultSet rs = pstmt.executeQuery();
        	DefaultTableModel tablemodel=new DefaultTableModel(); //表格模型
    ...
    public static JTable selectTotalNumAndTotalPriceOfOrderInfoByS_Category(String s_category, String shopkeeper_id){//这里的shopkeeperid来自登陆时的信息
    	conn = Connect.getConnection();
    	String sql = "select count(distinct op.order_id) as '该类商品订单数', sum(op.amount*p.price) as '该类商品订单总金额'  from OrderProduct op, Product p, OrderInfo oi  where op.order_id = oi.order_id and oi.state != '已取消' and  op.shopkeeper_id = '" + shopkeeper_id +  "' and p.product_id = op.product_id and p.s_category = '" + s_category + "' and p.shopkeeper_id = '" + shopkeeper_id +  "'";
    	try {
    		pstmt = conn.prepareStatement(sql);
        	ResultSet rs = pstmt.executeQuery();
        	DefaultTableModel tablemodel=new DefaultTableModel(); //表格模型
  ...
  //订单功能2.2：查询某中类商品的订单数和总金额
    public static JTable selectTotalNumAndTotalPriceOfOrderInfoByM_Category(String m_category, String shopkeeper_id){//这里的shopkeeperid来自登陆时的信息
    	conn = Connect.getConnection();
    	String sql = "select count(distinct op.order_id) as '该类商品订单数', sum(op.amount*p.price)  as '该类商品订单总金额'   from OrderProduct op, Product p, Category c, OrderInfo oi  where op.order_id = oi.order_id and oi.state != '已取消' and op.shopkeeper_id = '" + shopkeeper_id +  "' and p.product_id = op.product_id and p.s_category = c.s_category and c.m_category = '" + m_category + "' and p.shopkeeper_id = '" + shopkeeper_id +  "'";
    	try {
    		pstmt = conn.prepareStatement(sql);
        	ResultSet rs = pstmt.executeQuery();
        	DefaultTableModel tablemodel=new DefaultTableModel(); //表格模型
  ...
  //订单功能2.3：查询某大类商品的订单数和总金额
    public static JTable selectTotalNumAndTotalPriceOfOrderInfoByL_Category(String l_category, String shopkeeper_id){//这里的shopkeeperid来自登陆时的信息
    	conn = Connect.getConnection();
    	String sql = "select count(distinct op.order_id) as '该类商品订单数', sum(op.amount*p.price)  as '该类商品订单总金额'   from OrderProduct op, Product p, Category c, OrderInfo oi  where op.order_id = oi.order_id and oi.state != '已取消' and  op.shopkeeper_id = '" + shopkeeper_id +  "' and p.product_id = op.product_id and p.s_category = c.s_category and c.l_category = '" + l_category + "' and p.shopkeeper_id = '" + shopkeeper_id +  "'";
    	try {
    		pstmt = conn.prepareStatement(sql);
        	ResultSet rs = pstmt.executeQuery();
        	DefaultTableModel tablemodel=new DefaultTableModel(); //表格模型
  ...
  //订单功能3：查询某时间段内前十名热销商品
    public static JTable selectMostPopularProductByTime(String t_from, String t_to, String shopkeeper_id){//这里的shopkeeper_id来自登陆时的信息
    	...
    	String sql ="select p.product_id as '商品编号', p.product_name as '商品名', sum(op.amount)  as '该时间段订单数量'  "
    			...
    			+ "where oi.state != '已取消' and p.product_id = op.product_id and oi.order_id = op.order_id and oi.shopkeeper_id = '"
  ...
  //订单功能4：查询前十名热销商品
    public static JTable selectMostPopularProduct(String shopkeeper_id){//这里的shopkeeper_id来自登陆时的信息
    	...
    	String sql ="select p.product_id as '商品编号', p.product_name as '商品名', sum(op.amount)  as '订单数量'  "
    			...
    			+ "where  oi.order_id = op.order_id and oi.state != '已取消' and p.product_id = op.product_id and oi.order_id = op.order_id and oi.shopkeeper_id = '" + shopkeeper_id 
    			+ "' group by p.product_id, p.product_name order by sum(op.amount) desc limit 10";
    	try {
    		pstmt = conn.prepareStatement(sql);
        	ResultSet rs = pstmt.executeQuery();
  ...
  //订单功能5：查询评分前十名商品
    public static JTable selectBestScoreProduct(String shopkeeper_id){//这里的shopkeeper_id来自登陆时的信息
    	...
    	String sql ="select p.product_id as '商品编号',   p.product_name as '商品名',  avg(op.score) as '评分' "
    			+ " from product p, OrderProduct op,OrderInfo oi  where  oi.order_id = op.order_id and oi.state != '已取消' and p.product_id = op.product_id and p.shopkeeper_id = '" + shopkeeper_id 
